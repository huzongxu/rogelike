...
> 状态:已评审并全量落地(P1 主题层 + L2 赛季联动词缀 + P2/P3/P4 三套赛季限定套组 frost/magma/phantom,验收见 `docs/CONTEXT.md` 条目 40–43)。代码基线:2026-08-29(611 测试全绿,基线 = CONTEXT.md 条目 39)——**该数字是撰写时快照,当前基线 69 文件 / 3973 例**。
> 后续变更:英雄系统把 6 套扩到 12 套(每季 3 套专属),套组的**件数联动已由「共鸣数」里程碑接管**(原 bonus3 / bonus6 数值不变、触发条件改读本局共鸣 2 / 4 处),`featuredSetId` 保留为视觉与文案锚点;见 `docs/DESIGN-HERO-RHYTHM.md` §5 与 R5。
...
> 路径约定:本文盘点表与「改动面」表里的 `src/data/…` / `src/game.ts` 是撰写时路径;这些模块现住在共享层 `cocos/assets/scripts/game/`(经 `@game/*` 被两端 import),读位置按此映射。
...
| 微信包体(清单现 154 键;像素翻新后 web-desktop 整包 6,144,511 B、贴图 1,954,271 B / 158 枚) | 新效果图标走 icon_fx 管线各 +1 张 96px 小图(≈10KB),无大图;微信主包 4MB 的达标性待 Cocos `wechatgame` 出包时按平台侧复量 |
