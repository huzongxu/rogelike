...
> 状态：截至 2026-09-16（master `70d13af`），`docs/DESIGN-HERO-RHYTHM.md` §11 裁定 R1–R15 全部落地，`docs/CONTEXT.md` 53–68 是逐轮记录（64 = R15 攒下的真机验证清完，66 / 68 = 洛卡与穆的逐秒剖面，67 = 洛卡吞噬者特効落地）。本文件只收**还没做完的事**，做完一条就删一条；新增待办也记在这里，不散落在 CONTEXT 里。

## 1. 数值层（真机口径 = `chapterReset + chapterTypes`，3 种子 21 / 123 / 7）

- **穆精英章之墙 (杠杆 A 已落地，第 15 章修好、第 10 章仍在)**：10 / 15 / 21 → **10 / 21 / 21**（CONTEXT 68）。已做的是承伤反哺池：玩家承伤按 `MU_RETALIATION_POOL_PCT` 0.6 入池（上限 `maxHp × 0.5`、半衰期 2.5s），灵狼命中从池里抽取追补回血——**反哺总量恒 ≤ 承伤 × 比例，与狼数无关**（这一条是设计时特意防的 runaway）。两条剖面事实值得留着：① 卡点在**释放速率**而不是池子大小——`perHitMult` 1.5 时约 90 次命中/s 也只能退 ≈540/s，追不上单秒 734–1005 的精英承伤尖峰，提到 8 后第 15 章翻盘；② 第 10 章那一死**任何池参数都救不了**，因为致死脉冲发生在章首第 16 秒的第一波精英贴身，此时池里还没有可退的存量。剩余方向因此收窄为**开局脉冲本身**（站桩档在"精英已到脸"那一拍之前的脱离时机，或精英章开场的到场节奏），或接受穆 = 高风险高上限。
