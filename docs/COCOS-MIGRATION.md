...
**Cocos 侧接线点**：`GameShell.PENDING_SCREEN` 剩四条占位轻提示（`commission` / `gacha` / `talent` / `fusion`，菜单入口在 `onMenuAction` 分发、融合在商店工具钮分发），已落地屏逐屏换成真实 `router.show(...)`；`core/ScreenRouter.ts` 的 `SCREEN_KEYS` 为 16 态全量，路由实际注册 battle / menu / shop / heroes / leaderboard / daily / pass / gearup 八屏。`drawAvatarFrame` 住在 Web 侧 `src/ui/skin.ts`（Canvas2D），不在共享层 —— 排行屏的关卡框徽标按 Phase 3 立绘同款处置：贴图优先、缺图回退代码形状。
...
**本期尚未核实、动到该屏前要先测的**：扭蛋溢出的具体条数阈值（按 `spreadRows` 推算，未截图实测）；`startNewRun → restart` 是否必然使 `save.prestiges + 1`（`+1` 写在 `settleRun`，调用链未逐行核）；装备槽上限中 `extra_gear → runSlotBonus` 的注入路径；委托时长文案与 `COMMISSION_MAX_HOURS`、衰减曲线参数的一致性；`balance.json` 当前是否实际覆盖了 gacha 常量（`applyBalance` 钩子存在，覆盖值未查）。
