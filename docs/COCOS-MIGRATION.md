...
渐变贴图化已落地：Cocos 侧脚本里 `createRadialGradient` / `createLinearGradient` 现为 **0 处**（实测 `cocos/assets/scripts` 全树），需要渐变的场合一律走美术贴图 + `UIOpacity` 或 `Graphics` 描形，不引入 Shader。Web 冻结基准 `src/game.ts` 仍留 1 处，属基准面不动。
...
### 7.1 资源镜像
`npm run sync:cocos`（`scripts/sync-cocos.mjs`）把 `public/assets/*.png` → `cocos/assets/resources/textures/`，`public/config/*.json` → `resources/config/`。只新增与覆盖，不删除。PNG 的 `.meta` 由编辑器 asset-db 自动生成（含 texture + spriteFrame 两个 subMeta）。

贴图镜像已达全量 152/152（`cocos/assets/resources/textures/` 与 `public/assets/*.png` 同一套）；包体与首屏加载耗时的实测归 Phase 6（见 §9 风险 R2）。

### 7.2 命令行构建 + 本地服务
```bash
npm run build:cocos                      # CocosCreator --project cocos --build "platform=web-desktop"
...
**删改脚本后必须清 `temp/` 再构建。**命令行构建复用 `cocos/temp/programming/packer-driver/` 里的编译缓存与 import-map，而这些缓存只在编辑器进程内随 asset-db 重扫更新。在编辑器外删掉或改名一个 `.ts`（连同 `.meta`）后直接构建，日志照样报成功，产物里却仍带着旧模块——认得出来的特征是 bundle 里那个模块 id 等于被删 `.meta` 的 uuid 压缩串。做法：`rm -rf cocos/temp` 后重跑 `build:cocos`，再 grep `build/web-desktop/assets/main/index.js` 确认被删的导出名归零、保留的符号仍在。
...
`npm run typecheck:cocos`（`scripts/cocos-typecheck.mjs`）把 `cocos/assets/scripts/**/*.ts` 整棵树过一遍 `tsc --noEmit`，把类型检查补到门 2 与门 3 都够不着的那半棵树上：

- **覆盖面**：根 `tsconfig.json` 的 `include` = `src` / `tests` / `cocos/assets/scripts/game`，所以门 2 那次 `tsc --noEmit`（严格档、含 DOM）只看得到共享层；`game/` 之外的那半棵树 —— `core/`、`battle/`、各屏目录、`GameShell.ts`、`ui/PanelKit.ts` —— 只有门 3 的 CLI 构建会编译它们，而编译走打包器自己的 transpile，`error TS` 计数为 0 不代表做过类型检查。漏 import 一类的引用错误就落在这条缝里，只有实机进屏才炸。
- **口径**：`-p cocos/tsconfig.json` 继承编辑器生成的 `temp/tsconfig.cocos.json`（拿到 `cc` 的 `types` 与 `db://assets/*` 映射），命令行再覆盖三项 —— `--skipLibCheck`（引擎 `.d.ts` 自身不干净，屏蔽后工程外报错归 0）、`--strictNullChecks`（把档位抬到与根配置一致；`game/data/menuSkin.ts` 那条 TS2322 是缺这个开关造成的，抬上去就消失）、`--lib es2020,dom`（补 `window` / `localStorage` 之类宿主声明）。不取整档 `--strict`：那会给 `battle/HudView.ts` 的四枚 `@property` 字段带来 `TS2564` 误报（cc 组件的字段由编辑器注入）。
- **依赖门 3**：`temp/tsconfig.cocos.json` 由编辑器或命令行构建生成，`rm -rf temp` 后要等 `build:cocos` 跑完才有。缺它时本门直接给出这句话并退出，不去猜配置。
- **判据**：只数路径落在 `cocos/assets/` 下的报错，与脚本里的 `BASELINE` 逐条对账（键 = 文件 + 错误码 + 条数）。新增、条数超出基线、基线条目已失效这三种形态都判红，所以基线不会随时间只涨不落。
...
共享层位于 `cocos/assets/scripts/game/`，42 个文件：`core/math.ts`、`data/`（28 张表）、`entities/`（4）、`systems/`（`waves` `equipmentEngine` `onboarding`）、`ui/`（`theme` `hud` `menuLayout` `shop` `heroSelectLayout` `scrollList`）。两端读同一份源码。
...
验收（已逐项实测通过）：`npm run build`（Web，tsc `--noEmit` + vite build）与 `npm run build:cocos`（web-desktop）均通过；`npm test` 40 套件 / 1039 用例全绿；守卫测试对注入的 `from "cc"` 报错；`public/config/balance.json` 与 `cocos/assets/resources/config/balance.json` 逐字节相同（由 `npm run sync:cocos` 镜像）。共享层确实进入 Cocos 产物：`spreadRows` / `confirmRects` 出现在 `cocos/build/web-desktop/assets/main/index.js`。

### Phase 1 — 战斗主屏（已落地）

`cocos/assets/scripts/battle/` 六个模块：`BattleSim`（共享层适配器，注入 `FxLayerData` 作绘制桥）、`BattleWorldView`（实体渲染 + `cc.NodePool` 对象池）、`HudView`（顶坞 64 / 底坞 48，几何全部走 `game/ui/hud.ts`）、`FxView` + `FxCore`（`fxLayer` 的纯数据时间轴保留，只把"画"换成节点；含飘字池）、`JoystickView`（全屏 `TOUCH_START/MOVE/END` + 键盘）。`GameShell` 收敛为搭层级、装载资源、跑路由、交 dt 四件事。`viewTable.json` 扩 `battle`/`fx`/`pool`/`hud`/`joystick` 五段。
...
- **同一出口下的其余描边点（本轮并入）**：`ui/PanelKit.ts` 的 `qualityBox` 与 `Plate.show` 两处在此后的一笔里已并入同一条 `strokeRing` 出口，本文件全仓已无 `Graphics.stroke()` 调用点（只剩那条解释「为什么不走 stroke」的注释）。散在视图里的自绘矩形描边本轮一并收口：`leaderboard/LeaderboardView.ts` 的徽标缺图框、`menu/MenuLayoutView.ts` 的英雄带缺图回退框、`heroes/HeroSelectView.ts` 的详情板框 / 行立绘框 / 徽章框三处（`RING_LINE_W = 2`，按 −1.5 定律原本只剩半强度）全部改调 `strokeRing`，屏层已无 `Graphics.stroke()`。
- **战斗层同族（本轮新账，未动）**：收口屏层时数出 `battle/` 下还有同一条定律作用的自绘描边——`HudView.ts` 九处 `lineWidth = 1`、`BattleWorldView.ts` 四处 1px 与四处 2px、`FxView.ts` 两处 3px、`JoystickView.ts` 一处（线宽取 `viewTable.joystick.lineWidth = 2`）。当初这条挂账的 27 处计数只扫了各屏带描边实参的调用点，战斗层不在扫描面内，所以它是**新暴露的一面**而非旧账复发。没在翻新收尾这一笔里动它有三个理由：一是战斗层这些线多为细线与预警环，1px 档本身配的是低透明度色（如地面网格 `rgba(255,255,255,0.05)`），改与不改的成图差要在实机逐帧比对才判得准，而本笔的验收面是十七屏静态接触表；二是战斗层的绘制与 Web 走的是另一套热路径与节流，动它要重跑战斗指纹，与本轮的改动面不同源；三是这一族里 `JoystickView` 与预警环画的是**圆**，`strokeRing` 只拼矩形环带、够不着它们，收它们要另配一条环形实心 fill 出口，不是一次调用点替换。**待另单**，与 S3 一起按实机剖面收。
- **S3 文字基线整体偏低**：委托列表 35 行里 33 行比 Web 低 4~7px（22px 粗体标题差 −2）、面板态可分的 10 行低 4~6px、融合 18 行低 4~7px、赛季 6 行低 6~8px（钮题落到钮中心下方）。**仍挂账**：症状是同一批请求的基线在 Cocos 侧整体落低 4~8px 且随字号与粗细变化，收它要在 `placeLine` 一处做全局基线补偿，会同时挪动十七屏的每一行字并连带重基线各屏的几何断言——属跨屏改动，不适合放在翻新收尾这一笔里，且它只影响与 Web 的逐像素对标，不影响任何一屏内部的对齐与可读性。
- **S4 贴图被拉满节点盒（已核销）**：逐处复核绘制盒与源图尺寸后，四类里三类在 §13 全量换图时已按各自绘制盒 1:1 重出，不再存在非整数倍拉伸——立绘四处逐位相同（`player_pose_2`/`_6` 38×60 = `DL_DECO`/`CM_POSE`，`player_pose_4` 58×92 = `GO_POSE`，`player_pose_5` 44×60 = `PT_POSE`）、赛季徽标 48×40 与 `emblem_flow_gold` 源比例同源、碎片与星尘图标 14×14 方图入方盒。唯一仍走非整数倍的是委托屏的羊皮纸底板：`CM_PANEL_H` 从 150 抬到 158 之后 art 盒仍留 264×75，于是纵向被拉 1.053 倍、2 倍最近邻的像素格被拉歪。本轮把 art 盒改回与屏上矩形同源的 264×79（`artwork/pixel-kit-batch5.json`），重出后 528×158 与屏上逐位重合，本条闭合。
...
**这条反向变化同日闭合（首屏贴图分档）**：`ready` 由 3,115 ms 降到 **2,288 ms**（同一构建口径的中位数），比 §13 之前 22 枚预载世代的 2,452 ms 还早 164 ms；到 ready 为止的关键路径由 250 请求 / 4,014,576 B 降到 **106 请求 / 3,897,998 B**（其中 PNG 27 枚 = 180,042 B）。三条机制结论：

- 成本项确实是**枚数**：同一页内 A/B 实测，逐枚 `resources.load` 4.5 ms/枚、一次数组批量调用 5.5 ms/枚（24 枚、同缓存条件、两轮之间 `resources.release` 逼管线重跑）—— 换成数组口径不省钱，管线开销按资产计，唯一的杠杆是少等几枚。
- 「PNG 请求数」不能代理「预载枚数」：158 枚贴图整场只发 72~73 次 PNG 请求，小件被 `auto-atlas.pac` 打了图集。判枚数要看 `shell.frames.size`（实测稳态 158）。
- 分档口径：`ready` 前只 await `HUD_PRELOAD_KEYS`（27 枚，`buildLayers` 定格的坞板 / 徽章 / 横幅 / 装备卡图标），其余一律 ready 之后流式；流式队首固定是新增的 `BATTLE_FIRST_PAINT_KEYS`（像素数字字形 + 敌人 + 玩家本体 + 特效弹道，39 枚），其后才是像素批次其余件与清单里的世界美术。

三档刻度实测（冷缓存、560×996）：`ready` 2,288 → 战斗第一拍补齐 ready+262 → 全表到位 ready+813（3,096 ms）。画面完整的时间点没有变差（旧口径要到 3,115 ms 才建完界面），第一屏像素提前约 830 ms。稳态取证三张：ready+125 ms 的战场已是完整 HUD + 像素数字 + 战场底图，角色在 ready 后 0.3~0.9 s 入画（与 22 枚预载世代的既有行为同型，那时玩家立绘也在流式集里），3.2 s 后的战场与 3.6 s 的菜单逐件无缺。

守门：`tests/cocos-frame-stream.test.ts`（8 条）盯「三档差集必须为空」——队列计算因此从 `GameShell.restFrameKeys` 移进 `pixelArt.ts` 的纯函数 `streamFrameKeys`，这样「有没有键两头不沾」能被单测直接算。负例自检已做：摘掉 `push(PIXEL_ART_KEYS)` 后该断言立刻报出 13 枚「既不 await 也不流」的键。这一步不是形式——第一版 `restFrameKeys` 沿用了含 `PIXEL_ART_KEYS` 的旧过滤集，93 枚像素件两头不沾、`frames.size` 停在 65，源码守门与 `frames` 实机读数各抓到一半。
...
**异常白名单判据**（与 `scripts/smoke-cocos.mjs` 头注释同一份文字）：`loadFrames` null-frames `TypeError` 的触发条件已实测钉死，产品侧也已修好 —— 它是**场景重建撞上后台流式加载**的产物，不是每次启动必发：ready 之后 128 枚贴图在飞时页内 `loadScene('Main')` 销毁 GameShell（引擎 destroy 把实例字段置 null），在飞回调读 `this.frames.set(...)` 逐条抛 `TypeError: Cannot read properties of null (reading 'set')`；现在的写法是进 Promise 前捕获 `const sink = this.frames`，销毁后的回调只推进计数。实测四种形态：ready 点重载场景 → **128 条**（全部在飞）；p28 探针 harness（探针开头条件式 `loadScene('Main')` 与自举抢跑）→ **113 条**；**单次冷导航（冒烟与 p29 的形态）→ 0 条**；整页 reload（文档级销毁，回调不再执行）→ 0 条。白名单签名 = 该 TypeError 文本开头 + 首栈帧 URL 含 `/assets/main/index.js`；FAIL 只数白名单外，白名单命中单独计数打印 —— 修复后该计数实测为 0，白名单留着是为了这条通路一旦被改动仍能单独结账，而不是把判据混进 FAIL、也不是把"命中 0"当成"没有异常"。计数是报告值（0~128 随触发时在飞数浮动），不作硬断言。
