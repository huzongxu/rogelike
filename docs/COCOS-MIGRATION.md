...
**Cocos 侧接线点**：`GameShell.PENDING_SCREEN` 已随最后一屏（词缀融合）落地**整表删除**（占位轻提示 0 条 —— 删完那张表就空了，商店工具钮的 `fusion` 分支换成真实 `openFusion()`，不再留 `toast(PENDING_SCREEN[x])` 那类 `undefined` 路径）；已落地屏逐屏换成真实 `router.show(...)`。`core/ScreenRouter.ts` 的 `SCREEN_KEYS` 为 16 态全量，路由实际注册 battle / menu / shop / heroes / leaderboard / daily / pass / gearup / gacha / prestige / commission / fusion 十二屏。`drawAvatarFrame` 住在 Web 侧 `src/ui/skin.ts`（Canvas2D），不在共享层 —— 排行屏的关卡框徽标按 Phase 3 立绘同款处置：贴图优先、缺图回退代码形状。
...
### Phase 5 — 流程屏与覆盖层
`drawGameOver` `drawVictory` `drawConfirm` `drawSectionHeader` `drawSlamWarn` + `Overlay` 常驻层。（`drawTriplePanel` `drawHiddenChoice` 是融合屏自身的绘制件，已随 Phase 4 词缀融合屏落地，见上）
