...
**Cocos 侧接线点**：`GameShell.PENDING_SCREEN` 剩一条占位轻提示（`fusion`，在商店工具钮分发），已落地屏逐屏换成真实 `router.show(...)`；`core/ScreenRouter.ts` 的 `SCREEN_KEYS` 为 16 态全量，路由实际注册 battle / menu / shop / heroes / leaderboard / daily / pass / gearup / gacha / prestige / commission 十一屏。`drawAvatarFrame` 住在 Web 侧 `src/ui/skin.ts`（Canvas2D），不在共享层 —— 排行屏的关卡框徽标按 Phase 3 立绘同款处置：贴图优先、缺图回退代码形状。
