...
> 路径约定：本文的 `src/data/…` / `src/ui/…` / `game.ts:行号` 是撰写时事实；这些表与纯函数现已住在共享层 `cocos/assets/scripts/game/`（组合技在 `game/data/combos.ts`、地形在 `game/entities/objects.ts`、接线在 `game/systems/battleWorld.ts`），取值与行号以该树为准。
