...
- **数量**：每章 1–2 个（标定回调值；原设计 2–4 → 实测 2–3/1–2 档），第 1 章不生成（新手区净空，保 §3.3 新手台阶标定）；石柱:毒池 ≈ 2:1 随机。标定轨迹见 §3.4。
...
- 标定轨迹：2–4 个 → 全向弹幕 build 被石柱挡崩（新手局单种子回落超 15 波）→ 2–3 个仍崩（第 3 章抽到 3 石柱即雪崩）→ 1–2 个达标。数量是唯一回调旋钮（§3.4 纪律：不动生成曲线）；石柱双面性确认——既挡玩家弹幕也挡敌怪贴脸，弱局反而可能受益（曾见 9→21），故只比聚合不比单种子。
