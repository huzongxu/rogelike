...
**「安静地板」的可测下限** — 这条纪律原先只有文字、没有下限，于是同一批里出现了两档读法：
量法是把中央 55%×50% 区域（`bg_*` 上是 308×498）取 RGB 三道的标准差与量化到 4 bit 后的
色数，脚本 `.probe/p-px-bgcenter.mjs`（`node .probe/p-px-bgcenter.mjs bg_stage_1 ...`，
`PX_DIR` 可指向暂存目录先验后灌）。十枚背景实测：九枚落在 stdev 12~24 / 16~38 色，
`bg_stage_1` 只有 5.7/7.4/10.1 与 10 色，且源图中央带一圈可见的硬边界——单位在这条带上打，
但这条带本身读成了一个扣在地面上的空盒。故下限钉为：**中央带 stdev ≥ 12、量化色 ≥ 16，
与同批其余枚同档；且中央与四周之间不得出现硬边界**（安静靠低对比与窄明度实现，不靠无内容）。
`bg_stage_1` 已按此下限重出：同一 art 盒 560×996、同一 56 档量化表、同一 `"wm"` 与 `slide`
填充，源图换为地面连续铺满的墓地石砖（细节仍压两侧与两端），重出后中央带实测
13.2/16.1/21.0 与 19 色，均值 31.9/41.2/59.5 与旧档 32.3/42.2/62.4 同调，故不影响压暗与
`backdrop.coverAlpha` 那一层的既有判据。字节 162,300 → 214,641。
包体口径随之走一格：158 枚源贴图 1,954,271 → **2,002,885 B**、整包 6,144,511 →
**6,195,742 B**，枚数不变、源↔产物 158/158 md5 镜像口径不变，
故 `docs/COCOS-MIGRATION.md`「Phase 6 包体与首屏实测」那张分项表读作 2026-09-09 当日快照。
