...
一局的决策点约 20 个,**类型全部相同**(章间商店买卡);60 秒战斗内玩家零参与;数值墙在第 5-6 章,之后一局的走向基本可预判。肉鸽的爽感来自「决策类型多样 × 随机事件 × 高光时刻」三件事,目前三项都偏薄。英雄是纯皮肤,战斗过程没有可辨识的「我在玩谁」。

下列 8 条全部复用现有钩子,按性价比排序。「重标定」列指是否需要重跑 `balance-sim` 与相关测试锚点。

### 7.2 提案表

| # | 提案 | 解决什么 | 复用的现有钩子 | 新代码量 | 重标定 |
|---|---|---|---|---|---|
| 1 | **事件章**:20 章里 3-4 个章间商店换成随机事件(恶魔交易/赌徒/诅咒契约/废弃祭坛) | 决策类型单一 | `ChapterType` 加 `"event"` 档(`data/chapters.ts:11,64`);事件效果走 `EnvAffixDef` 同款数据结构(`data/envAffixes.ts:17,26`)与 `setBonus`/`comboActive` 同款 getter 折入(`battleWorld.ts:382`);UI 复用商店骨架 + 确认弹窗 | 中:1 张事件表 + 1 个事件屏(可套 `ConfirmView` 三选一) | 轻:章型表变了,`chapters.test`/`seasonPacing` 锚点要动 |
| 2 | **英雄主动技能**:每英雄 1 条大招,击杀充能;挂机自动放,手动模式按钮放 | 英雄纯皮肤;战斗零参与 | 充能挂 `onKill`(`equipmentEngine.ts:173`);释放 = 一次性 cast 现有效果的放大版(荆棘=全屏 nova、弹幕=时停齐射、余烬=陨星雨,`castMeteor/castNova` 已有);HUD 加 1 个按钮,复用摇杆侧 `JoystickView` 触控区 | 中:12 条技能参数 + 充能条 + 1 个按钮 | 中:大招 DPS 要进 sim(`set_*` 场景各加一项) |
| 3 | **第 10 章冠军怪**:带名字 + 2 条随机词缀(瞬移/吸金/分裂/护盾),必掉稀有以上卡 | 第 1-19 章情绪曲线平 | 精英章已固定 5/10/15(`chapterTypes.eliteChapters`);技能原语 11 种已参数化(`EnemyDef.skill`,`data/enemies.ts:180`、`seasonMonsters.ts`);掉落走 `victory()` 的装备掉落同款函数 | 小:1 行冠军 def + 词缀随机组合 + 名牌 UI | 轻:Boss 曲线不动,只验第 10 章 TTK |
| 4 | **挑战契约**:开局前勾选 1-3 条自我加压(禁回血/槽位减半/怪血 +50%),每条给回响与券倍率 | 3★ 后无重刷理由;审计 F3「券收入与投入无关」;体力无真实消耗场景 | 加压项 = 负向 `EnvAffixDef`,走 `stage.envAffixes` 同一入口(`data/stages.ts:34-36`);倍率折进 `victory()`/`settlePendingRun` 的奖励公式;开局前需要一页出征确认(顺带补上 §1.2 缺的确认页) | 中:契约表 + 出征页 + 奖励乘区 | 中:契约组合的可通关性要抽样跑 |
| 5 | **构筑发现时刻**:首次凑成 3 件套/6 件套/组合技/隐藏词缀 → 全屏横幅 + 0.4 秒时停 + 图鉴永久记录 | 已有的深度玩家感知不到 | 判定源 `COMBOS`(`data/combos.ts:36`)、`setBonus`、融合隐藏词缀;图鉴字段可放 `save.collection`(现 `collection.enemies` 只写不读,可一并盘活);时停复用 `frenzyTimer` 的帧内乘区思路(`battleWorld.ts:599-610`) | 小:表现层为主 | 无 |
| 6 | **章节规则变体**:每关随机 2-3 章带规则(巨型化/金币雨/暗夜/镜面反弹) | 章与章之间同质 | 全部是现有参数:`def.radius`×`speed`、`GOLD_PER_XP`、渲染可视半径、投射物边界处理(`updateProjectiles` 已有出界判定) | 小:1 张规则表 + HUD 一行提示 | 轻:单章级参数,不改曲线 |
| 7 | **局末高光 + 分享卡**:结算屏加「MVP 卡 / 最长连杀 / 本局发现」,生成分享图 | 20 分钟一局没有收尾;微信拉新 | 连杀已有 `comboCount`(`battleWorld.ts:1299`);MVP 需要在 `dmgLog` 里按装备聚合(现有伤害飘字通道可加 source 字段);分享走 `wx.shareAppMessage`(平台层预留位) | 小-中:统计聚合 + 1 张结算卡 | 无 |
| 8 | **无限关诅咒三选一**:每 5 波强制选 1 条诅咒,榜单记「几条诅咒打到几波」 | 无限关只是数值堆到死;冲榜无故事 | 诅咒 = 事件章的同一张表(与 #1 共用);赛季分里 `seasonBest` 口径(`data/season.ts:94`)加诅咒权重;幻影榜种子公式同步 | 小(若 #1 先做) | 中:F1/F2 榜顶可达性结论要重验(`seasonPacing.test`) |

### 7.3 落地顺序建议

1. **先做 #5 + #6 + #7**:纯表现层与参数表,不碰曲线,一到两周内能让「同一套数值」的体感明显不同。
2. **再做 #1 事件章**,它是 #4 和 #8 的数据基础(同一张「正/负向规则表」)。
3. **#2 英雄大招**单独排,因为要进模拟器重标定 12 套,且牵涉 HUD 触控布局。
4. **#3 冠军怪**可与 S1 怪物美术批次一起走,配表活。

### 7.4 暂不建议

- 委托系统加互动、扭蛋加品质档:属于养成深度,不解决「一局好不好玩」。
- 镜头移动 / 大地图:与「一屏竞技场」的既定决策冲突,且会推翻现有全部布局与刷怪标定。
