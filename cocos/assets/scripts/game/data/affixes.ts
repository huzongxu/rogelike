...
export interface EffectParams {
  ...
  healOnHit?: number;
  /**
   * 承伤反哺入池比例(>0 = 开;挂在召唤流核心上)。玩家每次承受伤害时把 `承伤 × 本值` 转入反哺池
   * (上限与半衰期见 combat.RETALIATION_HEAL),召唤物命中时从池里抽取追加回血。
   * 单位:比例(0.6 = 承伤的 60% 可被狼群追回来)。依据:穆在精英章死于"承伤是尖峰、回血是平滑流"的错配
   * (CONTEXT 68:第 10 章单秒承伤 734 对回血 501)。出处:docs/TODO-HERO-RHYTHM.md §1 穆条、杠杆 A。
   */
  retaliationHeal?: number;
  /**
