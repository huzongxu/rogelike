/**
 * 布局台数据模型(dev-only)—— Web 侧工作台 `src/dev/layoutWorkbench.ts` 驱动的纯函数层。
 *
 ...
 *
 * 这里只回答"手柄摆在哪、导出哪些字段";它调的 `menuLayout` 表两端运行期都读 ——
 * Cocos 侧 `menu/MenuLayoutView.ts` 经 `ui/menuLayout.ts` 派生几何,Web 侧主菜单同一份表。
 * 于是工作台拖一次,两端跟着变,手柄枚举/钳制/拾取/导出也只有这一份实现。
 *
