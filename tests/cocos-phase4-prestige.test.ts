...
describe("GameShell 的转生与天赋屏接线", () => {
  ...
  it("路由实际注册十六屏(SCREEN_KEYS 仍是 16 态全量)", () => {
    ...
    // 箭头里直接调 syncX 的那十一条(season / gameover 落地后各 +1;heroes 走 syncHeroes 也在其列)
  ...
  it("开始新轮回走既有的 restartRun;结算只发生在 restartRun 首行(Web restart() 同位)且排在开新局之前", () => {
    ...
    expect(restart.indexOf("settlePendingRun"), "结算必须排在开局请求之前,否则 startRun 会清掉挂起标记").toBeLessThan(restart.indexOf("requestStage"));
