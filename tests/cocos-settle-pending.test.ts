...
describe("宿主接线:三个 run-start 点都要结算在前", () => {
  it("GameShell 的选关 / 无限关 / 重开三处都在 start* 之前调 settlePendingRun", async () => {
    const { readFileSync } = await import("node:fs");
    const src = readFileSync(new URL("../cocos-prototype/assets/scripts/GameShell.ts", import.meta.url), "utf8").replace(/\r\n/g, "\n");
    expect(src.split("sim.settlePendingRun();").length - 1, "三个 run-start 点各一次真实调用").toBe(3);
    // 逐处确认结算排在开新局之前
