/**
 * 「完美蓝图」开局效果注入(BattleSim 的 starterEffect 通道)与开局装备的既有优先级。
 *
 ...
 *
 * 节律体系(docs/DESIGN-HERO-RHYTHM.md §3)下开局那一件是**核心技能**,落在 `player.skills`
 * 而不是主动法宝槽 `player.equipment`:未选英雄时核心 = 宿主初始武器标为技能;选了英雄时核心
 * 来自该英雄的技能表;随身收藏件进主动槽并按本局本命节律归一化。
 */

import { describe, it, expect } from "vitest";
import { readFileSync } from "node:fs";
import { vec2 } from "@game/core/math";
import { makeStarterEquipment } from "@game/data/equipmentGen";
import { coreSkillOf } from "@game/data/heroSkills";
import { HERO_RHYTHM } from "@game/data/rhythm";
...
describe("完美蓝图:开局武器效果由宿主注入(落为核心技能)", () => {
  it("不注入时仍是飞刀,且带工厂里那组飞刀专属参数;开局只有这一件核心技能、主动槽为空", () => {
    const sim = makeSim();
    expect(sim.player.skills.length, "无收藏件无英雄时开局只带一件核心技能").toBe(1);
    expect(sim.player.equipment.length, "主动法宝槽开局为空(首章三选一才发第一件)").toBe(0);
    expect(sim.player.skills[0]).toMatchObject(makeStarterEquipment("knife"));
    expect(sim.player.skills[0].kind).toBe("skill");
    const p = sim.player.skills[0].effect.params;
    expect([p.spread, p.pierce, p.damage]).toEqual([16, 1, 40]);
    expect(sim.player.rhythms, "未选英雄 → 本命节律为周期").toEqual(["pulse"]);
  });
  ...
  it("注入哪个效果就带哪个:名字、触发器与参数全走工厂同一条路", () => {
    ...
    const starter = sim.player.skills[0];
    expect(starter).toMatchObject(makeStarterEquipment("nova"));
  ...
  it("转生屏那八枚效果钮逐个都能落到核心技能上", () => {
    expect(PT_EFFECT_TYPES.length).toBe(8);
    for (const type of PT_EFFECT_TYPES) {
      const sim = makeSim(() => type);
      expect(sim.player.skills[0].effect.def.type, `按钮 ${type} 与战斗层消费的 EffectType 对不上`).toBe(type);
    }
  });
  ...
  it("每次开局现取:改了选装,下一局就换武器(Web 的 runConfig 从不快照)", () => {
    ...
    expect(sim.player.skills[0].effect.def.type).toBe("nova");
    chosen = "chain";
    if (!sim.startStage(1)) throw new Error("第二次 startStage(1) 未开局");
    expect(sim.player.skills[0].effect.def.type, "第二次开局要读到改后的值").toBe("chain");
    chosen = undefined;
    if (!sim.startStage(1)) throw new Error("第三次 startStage(1) 未开局");
    expect(sim.player.skills[0].effect.def.type, "取消选装后回落飞刀").toBe("knife");
  });
  ...
  it("不拥有「完美蓝图」天赋也照样生效:门在 UI 侧,战斗层不复查(Web 同形)", () => {
    ...
    expect(sim.player.skills[0].effect.def.type).toBe("skeleton");
  });

  it("选了英雄时核心技能来自技能表、本命节律来自英雄表,宿主注入的效果不参与", () => {
    const save = emptySave();
    save.energy = 99;
    ...
    const core = coreSkillOf("vera");
    expect(sim.world.heroId(), "只写 selectedSet 时由套组反查英雄(双射)").toBe("vera");
    expect(sim.player.skills).toHaveLength(1);
    expect(sim.player.skills[0]).toMatchObject({ kind: "skill", skillId: core.id, name: core.name });
    expect(sim.player.skills[0].effect.def.type).toBe(core.effect);
    // 首条 = 本命节律;事件节律核心另带一条「底拍」慢周期(R6 冷启动保底)
    expect(sim.player.skills[0].triggers.map((t) => t.def.type)).toEqual([core.rhythm, "pulse"]);
    expect(sim.player.rhythms).toEqual([HERO_RHYTHM.vera]);
    expect(sim.player.equipment).toHaveLength(0);
  });

  it("带了收藏件时它进主动槽并按本命节律归一化,核心技能照常在(未选英雄 → 核心仍按宿主注入给)", () => {
    const save = emptySave();
    save.energy = 99;
    ...
    if (!sim.startStage(1)) throw new Error("startStage(1) 未开局");
    expect(sim.player.equipment).toHaveLength(1);
    expect(sim.player.equipment[0].effect.def.type, "收藏件进主动槽").toBe("ray");
    expect(sim.player.equipment[0]).toMatchObject({ id: owned.id, kind: "active" });
    expect(sim.player.equipment[0].triggers.map((t) => t.def.type), "挂在已解锁节律上").toEqual(["pulse"]);
    expect(sim.player.skills[0].effect.def.type, "核心技能按宿主初始武器给(收藏件不再替代核心)").toBe("nova");
    expect(sim.player.skills[0].kind).toBe("skill");
  });
