...
export function buildSkinTree(skin: MenuSkinTable): SkinTreeNode[] {
  return SKIN_TREE.map((p) => ({
    ...
    layers: p.layers.map((l) => {
      ...
      for (const k of l.assetKeys) resolved[k] = skin.remap[k] ?? k;
      const visible =
        l.kind === "image" ? l.assetKeys.every((k) => !skin.hidden.includes(k)) : !skin.textHidden.includes(p.id);
      return {
        layerId: l.id,
        kind: l.kind,
        ...
        resolved,
        insets: { ...(skin.insets[p.id] ?? ZERO_INSETS) },
      };
    }),
  }));
}
...
export function exportSkinJson(skin: MenuSkinTable): string {
  ...
  if (raw.insets && typeof raw.insets === "object") {
    for (const pid of MENU_PANEL_IDS) {
      ...
      for (const s of SIDES) if (isNum(rec[s])) ins[s] = clampInset(rec[s]);
