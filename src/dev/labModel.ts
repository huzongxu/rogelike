/**
 * 布局台数据模型(dev-only)—— `src/dev/layoutWorkbench.ts` 与 `tests/layout-lab-model.test.ts` 共用的纯函数层。
 *
 * 本文件**不碰 DOM、不碰游戏实例**:输入 `menuLayoutPure` 的产物,输出"手柄/参考框/导出 JSON"。
 * 之所以把锚点计算独立出来单测:手柄坐标错一位,布局台就会把用户引向"拖对了字段、摆错了位置",
 * 而这正是本项目要消灭的失败模式(说不清往左一点)。
 *
...
/** 生成可直接并入 public/config/balance.json 的 `menuLayout` 段 */
...
export function buildGuides(L: MenuLayout, ctx: HandleContext): Guide[] {
  ...
  if (makeupIdx >= 0) {
    ...
    // 命中与绘制同源:game.ts 补星命中读 L.makeupRect(提交3a 已拍板统一)
